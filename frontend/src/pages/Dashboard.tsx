...
const qualityData = MONTHS.map((m, i) => ({
  month: m,
  quality: Math.max(60, Math.min(100, 82 + Math.sin(i * 0.8) * 12 + i * 0.5)),
}))

// Country spend breakdown for donut
const COUNTRY_SPEND = [
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="grid grid-cols-5 gap-4 mb-6">
          ...
          <StatCard label="Active Connections" value={`${activeConns} / ${MOCK_CONNECTIONS.length}`}
            trendLabel="3 pending installation" icon={<Activity />} accent="amber" />
          <StatCard label="Budget Utilization" value="73%" trend="↑ 2.1%" trendUp={true}
            trendLabel={`${cfg.currencySymbol} 76K remaining`} icon={<PieChart />} accent="purple" />
          <div className="card">
              ...
              <tbody>
                {MOCK_CONNECTIONS.slice(0, 5).map(c => (
                  <tr key={c.id} className="tbl-row">
                    ...
                    <td className="tbl-td font-medium text-white/80">{cfg.currencySymbol} 24,180</td>
                    <td className="tbl-td"><button className="btn-sm flex items-center gap-1"><BarChart3 size={11}/> View</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
